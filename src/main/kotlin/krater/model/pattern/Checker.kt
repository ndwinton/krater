...
class Checker(val a: Color, val b: Color, transform: Matrix = IDENTITY_4X4_MATRIX) : Pattern(transform) {

    override fun colorAt(point: Tuple): Color =
        if (((floor(point.x) + floor(point.y) + floor(point.z)) % 2.0).near(0.0)) a else b
